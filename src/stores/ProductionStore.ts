...
import type { ProductionCurve, ProductionCurveDto } from '../types/Production'
...
export const useProductionStore = defineStore({
  ...
  state: () => {
    return {
      ...
      totalProduction: Array.from({ length: 96 }).fill(0) as number[],
    }
  },
  actions: {
  ...
  getters: {
    ...
    getAllProductionCurves: state => () => {
      const allProductionCurves: ProductionCurve[] = []
      for (const curve of state.productionCurves.values())
