...
import { getMaxUsableEnergyAmountPossibleOverPeriod, getMaxUsableEnergyAmountPossibleOverPeriodWithoutConsumption } from '../helpers/listProcessor'
...
export const useEnergyStore = defineStore({
  ...
  state: () => {
    return {
      ...
      availableStoredEnergyList: Array.from({ length: 96 }).fill(0) as number[],
  ...
  actions: {
    ...
    clickOnEnergyIcon() {
      this.clickedEnergyIcon = !this.clickedEnergyIcon
    },
    clickOnStoreEnergy() {
      this.clickedStoreEnergy = !this.clickedStoreEnergy
    },
    clickOnConsumeEnergy() {
      if (this.storedEnergy > 0)
        this.clickedConsumeEnergy = !this.clickedConsumeEnergy
    },
    clickOnMarketIcon() {
      this.clickedMarketIcon = !this.clickedMarketIcon
    },
  },
  getters: {
    getEnergyIcon() {
